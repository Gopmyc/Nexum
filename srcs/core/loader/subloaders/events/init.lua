...
function SUBLOADER:Initialize(tContent)
	assert(istable(tContent),			"[EVENT SUB-LOADER] Content must be a table")
	assert(isfunction(self.GetLoader),	"[EVENT SUB-LOADER] Loader access method is missing")

	-- self.__ENV.CONTENT.EVENT.LIBRARIES	= self:GetLoader():GetLibrariesBase("libraries", self.__ENV.CONTENT.EVENT)
	self.__BUFFER						= {}
...
function SUBLOADER:LoadFile(tFile, fChunk)
	...
	if not istable(tDependencies) and (#tFile.ARGS > 0) then 
		return MsgC(
			...
			"[EVENT SUB-LOADER] The dependencies for the file '" ..tFile.KEY.. "' could not be resolved."
		)
	end
