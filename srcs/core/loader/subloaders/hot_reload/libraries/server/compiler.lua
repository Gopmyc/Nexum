--[[--------------------------------------------------------------------
    ...
    Yueliang is licensed under the terms of the MIT license reproduced
    below. This means that Yueliang is free software and can be used
    for both academic and commercial purposes at absolutely no cost.

    Yueliang is based on Lua 5 code. See COPYRIGHT_Lua5 (Lua 5.0.3) and
    COPYRIGHT_Lua51 (Lua 5.1.3) for Lua 5 license information.
...
luaX.RESERVED = [[
	...
	function luaY:leaveblock(fs)
		...
		if bl.isloop then bl.continuelist = luaK.NO_JUMP end
	end
	...
	function luaY:continue_stat(ls)
		local fs = ls.fs
		local bl = fs.bl
		...
		while bl do
			if bl.isloop then
				...
				bl.continuelist = luaK:concat(fs, bl.continuelist or luaK.NO_JUMP, j)
				return
			end
			bl = bl.previous
		end

		luaX:syntaxerror(ls, "'continue' not inside a loop")
	end
	...
	function luaY:repeatstat(ls, line)
		...
		bl1.isloop = true
		bl1.continuelist = luaK.NO_JUMP
		luaX:next(ls)
		self:chunk(ls)
		self:check_match(ls, "TK_UNTIL", "TK_REPEAT", line)
		local condexit = self:cond(ls)
		luaK:patchlist(fs, bl1.continuelist, repeat_init)
	...
	function luaY:forstat(ls, line)
		...
		luaK:patchlist(fs, bl.continuelist, fs.pc - 1)
	...
	function luaY:statement(ls)
		...
		elseif c == "TK_BREAK" then
			...
			self:breakstat(ls)
			return true
		elseif ls.t.token == "TK_CONTINUE" then
			luaX:next(ls)
			self:continue_stat(ls)
		else
			self:exprstat(ls)
			return false
		end
...
function LIBRARY:Compile(sFilePath, sFileName)
	if CLIENT then return end

	local LuaState = {}

	----- Retrieving file contents -----
	local bSuccess, sFileContent	= xpcall(function(sFilePath)
		assert(isstring(sFilePath), "[SUBLOADER:VM:COMPILER] 'Compile' method exepted a string for argument #1")
		return lovr.filesystem.read(sFilePath, -1)
	end, function(sErr)
		MsgC(
	...
	----- Content compilation and error recovery -----
	local _, sByteCode	= xpcall(function(sFileContent, sFileName)
		assert(isstring(sFileContent), "[LIBRARY COMPILER] Expected string for source")
		sFileName	= sFileName or 'compiled-lua'

		local zio			= luaZ:init(luaZ:make_getF(sFileContent), nil)
		if not zio then return end

		local func			= luaY:parser(LuaState, zio, nil, "@"..sFileName)
